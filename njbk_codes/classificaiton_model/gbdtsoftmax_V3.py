# -*- coding: utf-8 -*-
"""
Created on Sat Jan  5 11:38:16 2019
...
#file = 'data/processed.csv' #binary data1
file = 'data/BreastTissue.csv'#multi data1
#file = 'data/glass.data'#multi data2
num_leaves = 20
num_classes = 6
...
# reshape(-1,num_classes) 后的数据是这样，本来是一个长向量 。       
...
#    多分类模型 1  
    ...
    X,X_train,X_test,y_train,y_test = nj.dataSplit(nj,df,'Class',0.2)#数据没有经过标准化
...
#    X,X_train,X_test,y_train,y_test = nj.dataSplit(nj,df,10,0.4,Standardization=True)
    ...
    #multi 1
    nj.roc_curve_Plot(y_test,y_score,'Class')
    nj.pr_curve_Plot(y_test,y_score,'Class')
    #multi 2
